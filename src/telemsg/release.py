...
用法::
    ...
    telemsg build --target windows                    # 推荐：作为 CLI 子命令
    python -m telemsg.release --target windows        # 直接跑模块
    python scripts/build_release.py --target windows  # 兼容旧写法（薄封装）
"""
...
def find_project_root(start: Path | None = None) -> Path:
    ...
    打包必须对着源码树执行——构建脚本、模板与源码都不随 wheel 分发，
    所以这里主动向上查找，而不是默认「当前目录就是根目录」。
    """
    override = os.environ.get("TELEMSG_SOURCE_ROOT")
    if override:
        candidate = Path(override).expanduser().resolve()
        if (candidate / "packaging").is_dir():
            return candidate
        raise BuildError(f"TELEMSG_SOURCE_ROOT={override} 下没有 packaging/ 目录")

    here = (start or Path(__file__)).resolve()
    for parent in [here, *here.parents]:
        if (parent / "pyproject.toml").is_file() and (parent / "packaging").is_dir():
            return parent
    raise BuildError(
        "找不到项目根目录：打包需要在源码仓库里执行，"
        "当前路径及其父目录中都没有同时包含 pyproject.toml 与 packaging/ 的目录。"
        "也可以用环境变量 TELEMSG_SOURCE_ROOT 显式指定。"
    )
...
def build_targets(
    ...
    *,
    ...
    if resolved in {"windows", "all"}:
        ...
        built.append(stage)
    if resolved in {"macos", "all"}:
        built.append(build_macos(out_dir=out_dir, make_zip=make_zip))
    return built


def describe_build_environment(target: str) -> None:
    print(f"\033[1mTeleMsgCreator 打包\033[0m  版本 {project_version()}  目标 {resolve_target(target)}")
    print(f"源码目录：{ROOT}")
